...
| `MERIDIAN_CRDT_GC_GRACE_MS` | `60000` | Grace period before GC removes old CRDT versions (ms) |

**PostgreSQL / pg-sync** (`--features pg-sync`):

| Variable | Default | Description |
|----------|---------|-------------|
| `DATABASE_URL` | *(unset)* | PostgreSQL URL — enables pg-sync and Postgres storage |
| `PG_SYNC_DATABASE_URL` | *(unset)* | Alias for `DATABASE_URL` |
...
**Clustering** (`--features cluster` or `cluster-http`):

| Variable | Default | Description |
|----------|---------|-------------|
| `REDIS_URL` | *(unset)* | Redis URL — enables Redis Pub/Sub cluster transport |
| `MERIDIAN_PEERS` | *(unset)* | Comma-separated peer URLs — enables HTTP cluster transport |
| `MERIDIAN_NODE_ID` | *(auto)* | Unique node ID — derived from `hostname:port` hash if unset |
| `MERIDIAN_INTERNAL_BIND` | `0.0.0.0:3001` | Internal bind address for inter-node HTTP sync (`cluster-http` only) |
