...
<Note>
  The `getMeridianTools` / `executeMeridianTool` helpers currently return **Anthropic-compatible** tool definitions. OpenAI and Gemini adapters are planned — see the [Roadmap](https://github.com/Chahine-tech/meridian/blob/main/ROADMAP.md).
</Note>
...
A full working example is in [`examples/ai-agents/agents/memory.ts`](https://github.com/Chahine-tech/meridian/tree/main/examples/ai-agents/agents/memory.ts). It uses three CRDTs (namespace: `ai-agents`):
...
A full working example is in [`examples/ai-agents/agents/debate.ts`](https://github.com/Chahine-tech/meridian/tree/main/examples/ai-agents/agents/debate.ts). It runs two Claude agents (FOR / AGAINST) in interleaved rounds, with each agent's argument history accumulated in an ORSet. A React UI displays both sides live.

## Live UI

The [AI agents example](https://github.com/Chahine-tech/meridian/tree/main/examples/ai-agents) ships a React dashboard with four tabs — **Agents** (multi-worker coordinator), **Tool Use** (single Claude agent), **Memory** (persistent memory across runs), and **Debate** (two-agent debate) — all subscribing to the same CRDTs via WebSocket and displaying live state as agents write.
