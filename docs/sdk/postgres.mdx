...
```sql
-- Total views across all articles — returns BIGINT directly
SELECT meridian.gcounter_total(views) AS total_views
FROM articles;

-- If you need the merged BYTEA state (e.g. for further merging):
SELECT meridian.gcounter_merge_agg(views) AS merged_state
FROM articles;
```
