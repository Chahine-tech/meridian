...
Tree node values support end-to-end encryption. Pass an `encryption` map to `MeridianClient.create()`:

```ts
import { Effect } from "effect";
import { MeridianClient, importAesGcmKey } from "meridian-sdk";

const key = await importAesGcmKey(process.env.DOC_KEY!);
...
console.log(outline.value().roots[0].value); // "Updated content" — decrypted locally
```

`addNode` and `updateNode` encrypt the `value` argument before sending. `deleteNode` and `moveNode` do not involve a value and are unaffected. The decrypted value is transparent via `value()`, `onChange()`, and `stream()`. See [Encryption](/sdk/encryption) for key generation and distribution patterns.
